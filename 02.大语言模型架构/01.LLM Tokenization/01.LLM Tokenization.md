# 01.LLM Tokenization：从文本到 token id

这一节讨论的是**大模型里的 tokenization**，也就是把原始文本转换成模型可以处理的 token / token id。
...
LLM 不直接处理“文字本身”，而是处理 token id。前面提到的 next-token prediction，本质上也是：

```text
根据已有 token ids，预测下一个 token id 的概率分布
```

因此，tokenization 是连接“用户输入文本”和“模型内部计算”的第一步。
