...
激活函数和分词 / tokenization 不是同一层概念：

```text
分词 / tokenization：把用户文本转换成 token ids
Embedding：把 token ids 转换成向量
Transformer：对 token 向量做上下文建模
激活函数：出现在 Transformer 内部，让模型具备非线性表达能力
```

在 Transformer Block 中，激活函数主要出现在 **FFN / MLP** 部分：

```text
输入 token 向量
↓
Self-Attention：不同 token 之间交换信息
↓
FFN / MLP：对每个 token 的表示做进一步变换
...
```text
tokenization 解决“文本如何进入模型”
attention 解决“token 之间如何互相参考”
激活函数解决“模型如何表达复杂的非线性关系”
```

对 LLM 学习来说，本节不需要重点深挖反向传播推导，重点理解：

```text
为什么需要非线性
为什么早期常见 ReLU
为什么 Transformer / BERT / GPT 常见 GELU
为什么 LLaMA / PaLM 等模型会使用 GLU / SwiGLU 变体
```

### 1.激活函数作用

如果神经网络只有线性层，那么无论堆叠多少层，本质上仍然可以等价为一个大的线性变换：
...
这种模型表达能力有限，难以学习自然语言中的复杂关系，例如词义、上下文、语法结构、代码结构、推理模式等。

加入激活函数，就是在网络中引入**非线性能力**，让模型可以表达更复杂的映射关系。
...
在大模型里，激活函数通常不是单独拿出来使用，而是作为 Transformer 层中的一个组件，参与每层 token 向量的变换。
...
如果目标是理解和使用大语言模型，不需要把每个激活函数的数学推导都背下来。更重要的是知道它们在模型架构中的位置和取舍。

#### 9.1 激活函数在 Transformer 哪里？

在 Transformer 中，激活函数主要位于 FFN / MLP 层：

```text
Transformer Block
├─ Self-Attention：让 token 之间交换上下文信息
└─ FFN / MLP：对每个 token 的向量做非线性变换
   ├─ Linear
   ├─ Activation
   └─ Linear
```

Self-Attention 更像是在解决：

```text
当前 token 应该关注哪些上下文 token？
```

FFN / MLP 加激活函数更像是在解决：

```text
每个 token 的表示应该如何进一步加工和变换？
```
...
| 激活函数 | 典型位置 / 模型 | 重点理解 |
| --- | --- | --- |
| Sigmoid / Tanh | 传统神经网络、RNN / LSTM | 容易梯度消失，更多作为背景知识 |
| ReLU | 早期深度网络、早期 Transformer FFN | 简单高效，但有 Dead ReLU 问题 |
| GELU | BERT、GPT 等 Transformer 模型 | 比 ReLU 更平滑，是 Transformer 中很常见的选择 |
| Swish / GLU / SwiGLU | PaLM、LLaMA 等现代 LLM | 引入门控机制，常用于替代传统 FFN 激活 |

#### 9.3 为什么 LLM 常见 GELU / SwiGLU？

GELU 可以理解为 ReLU 的平滑版本，不是简单地把负值全部置零，而是根据输入大小进行更柔和的保留或抑制。这种方式在 Transformer 中表现较好，所以 BERT、GPT 等模型中经常看到 GELU。

GLU / SwiGLU 则引入了“门控”思想：
...
这和 LSTM 里的门控有点类似，但它被用在 Transformer 的 FFN / MLP 中。现代 LLM 使用 SwiGLU，通常是为了提升模型表达能力和训练效果。

#### 9.4 面试表达

如果面试官问“激活函数在大模型里有什么作用？”，可以这样回答：

> 激活函数主要出现在 Transformer Block 的 FFN / MLP 部分，用来给模型引入非线性表达能力。没有激活函数，多层线性层叠加后仍然等价于线性变换，表达能力有限。早期模型常用 ReLU，BERT、GPT 等 Transformer 常见 GELU，LLaMA、PaLM 等现代 LLM 常见 SwiGLU 这类门控激活。

一句话总结：

> 分词把文本变成 token，Embedding 把 token 变成向量，Attention 让 token 之间交换信息，激活函数则让 Transformer 内部具备复杂的非线性表达能力。
